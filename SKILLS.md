...
description: Agent skills for working with Minions Test-observability MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-test-observability data.
---

# Minions Test-observability Agent Skills

Skills for agents operating on the `minions-test-observability` toolbox.
...
```bash
# TypeScript
pnpm add @minions-test-observability/sdk
...
The `test-observability` CLI provides basic project info and utilities:

```bash
# Show project info (SDK name, CLI name, Python package)
test-observability info
```
...
```ts
import { customTypes } from '@minions-test-observability/sdk/schemas';
...
```python
from minions_test_observability.schemas import custom_types
...
- This agent only writes to `minions-test-observability` — it reads from other toolboxes but never writes to them
